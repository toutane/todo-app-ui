...
import {
  ButtonGroup, Button, Card, CardTitle, CardFooter, Collapse,
  Row, Col, Modal, ModalHeader, ModalBody, ModalFooter,
  Form, FormGroup, Label, Input, FormText, Dropdown,
  DropdownToggle, DropdownMenu, DropdownItem,
  ButtonDropdown, CardText, Alert, InputGroupAddon, InputGroup,
  Popover, FormFeedback, Badge
} from 'reactstrap';
...
class TasksPanel extends React.Component {
  ...
  render() {
    ...
    const filteredTasks = this.state.tasks.filter(
      tasks => tasks.tasks_title.toLowerCase().includes(this.state.search.toLowerCase())
    )
    ...
    return (
      <div>
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; */}
              <InputGroup>
                <InputGroupAddon><i className="fa fa-search fa-fw" /></InputGroupAddon>
                <Input
          ...
          <Button color="info" onClick={this.addTasksModal}><i className="fa fa-plus" />&nbsp;Add a tasks</Button>
          <Button color="secondary" onClick={this.moreInformationFunction1}><i className="fas fa-ellipsis-v text-info" /></Button>
          <Button color="danger" onClick={null}><i className="fa fa-trash" /></Button>
        </ButtonGroup>
        {/* <ProjectStatue/> */}
        <Modal isOpen={this.state.addTasksModal} toggle={this.addTasksModal} >
          <ModalHeader><i className="fa fa-plus text-info" />&nbsp;Add a tasks</ModalHeader>
          <ModalBody>
            <Form>
              <FormGroup>
              ...
              <Button color="success" onClick={this.personalizationCollapse}>
                <i className="fas fa-paint-brush"></i>&nbsp;Personalization
              </Button>
            </Form>
            <Form>
              <Collapse isOpen={this.state.advancedOptionsCollapse}>
                ...
                <Card block outline color="info">
                  <Row>
                    <Col xs="6" sm="4">
                      <ButtonDropdown isOpen={this.state.dropdownSelectProjectOpen} toggle={this.dropdownSelectProjectToggle}>
                        <DropdownToggle caret outline color="info">
                          <i className={this.state.dropSelectItemIcon}></i> {this.state.dropSelectProject}
                        </DropdownToggle>
                        <DropdownMenu>
                          {projects.map((project, i) => <DropdownItem onClick={() => this.onAddProjectSelected(project)} key={i}
                          ><i className={project.project_icon} />&nbsp;{project.project_name}</DropdownItem>)}
                        </DropdownMenu>
                      </ButtonDropdown>
                    </Col>
                    <Col xs="6" sm="4">
                      <ButtonDropdown isOpen={this.state.dropdownSelectPriorityOpen} toggle={this.dropdownSelectPriorityToggle}>
                        <DropdownToggle caret outline color="info">
                          <i className={this.state.prioritySelectIcon} /> Priority
                        </DropdownToggle>
                        <DropdownMenu>
                          {priorities.map((priority, i) => <DropdownItem onClick={() => this.showPrioritySelected(priority)} key={i}
                          ><i className={priority.priority_icon} />&nbsp;{priority.priority_name}</DropdownItem>)}
                        </DropdownMenu>
                      </ButtonDropdown>
                    </Col>
                    <Col sm="4">
                      <ButtonDropdown isOpen={this.state.dropdownSelectTagOpen} toggle={this.dropdownSelectTagToggle}>
                        <DropdownToggle caret outline color="info">
              ...
              <Card block outline color="success">
                  <Row>
                    <Col></Col>
                    <Col>
                      <ButtonDropdown isOpen={this.state.dropdownSelectColorOpen} toggle={this.dropdownSelectColorToggle}>
                        <DropdownToggle caret outline color="success">
                          <i className={this.state.dropSelectColorIcon}></i> {this.state.dropSelectColor}
                        </DropdownToggle>
                        <DropdownMenu>
                          {colors.map((color, i) => <DropdownItem onClick={() => this.showColorSelected(color)} key={i}
                          ><i className={color.color_icon} />&nbsp;{color.color_name}</DropdownItem>)}
                        </DropdownMenu>
                      </ButtonDropdown>
                    </Col>
                    <Col>
                      <ButtonDropdown isOpen={this.state.dropdownSelectIconOpen} toggle={this.dropdownSelectIconToggle}>
                        <DropdownToggle caret outline color="success">
                          <i className={this.state.dropSelectIconIcon}></i> {this.state.dropSelectIcon}
                        </DropdownToggle>
                        <DropdownMenu>
                          {icons.map((icon, i) => <DropdownItem onClick={() => this.showIconSelected(icon)} key={i}
                          ><i className={icon.icon} />&nbsp;{icon.icon_name}</DropdownItem>)}
                        </DropdownMenu>
                      </ButtonDropdown>
                    </Col>
                    <Col></Col>
          ...
          <ModalFooter>
            <Button color="secondary" onClick={this.addTasksModal}>Cancel</Button>
            <Button outline color="info" onClick={this.addTasksFunction}><i className="fa fa-plus" />&nbsp;Add</Button>
          </ModalFooter>
        </Modal>
          ...
          <div key={i}>
            <Card block color={tasks.tasks_card_color}>
              <div className="d-flex justify-content-between align-items-start">
                <div>
                  <CardTitle>
                    <div><i className={tasks.tasks_card_icon} />&nbsp;{tasks.tasks_title}</div>
                  </CardTitle>
                  <CardText onClick={() => this.moreInformationFunction(i)}>
                    <i className="fas fa-ellipsis-v fa-sm text-info" />&nbsp;&nbsp;{tasks.tasks_description}&nbsp;
                    <Collapse isOpen={(i === this.state.selectedTasksInformation) && (this.state.activeTasksInformation) || (this.state.allActiveTasks)}>
                      <hr className="my-2" />
                      <h6>{tasks.tasks_date}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i className={tasks.tasks_project_icon} />&nbsp;{tasks.tasks_project}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i className={tasks.tasks_priority} /></h6>
                    </Collapse>
                  </CardText>
                </div>
                <ButtonGroup
                  className={
                    (this.state.visibleCross && (i === this.state.confirmDeleteTasks))
                  ...
                  {( this.state.visibleCross && (i === this.state.confirmDeleteTasks) ) 
                  ? (<ButtonGroup>
                      <Button color="danger" outline size="sm" onClick={() => this.deleteTasksFunction(i)}>confirm</Button>
                      &nbsp;<Button color="info" outline size="sm" onClick={() => this.canceVisibleButtons()}>cancel</Button>
                    </ButtonGroup>)
                  : <i className="fas fa-times fa-lg mb-1" onClick={() => this.confirmDeleteFunction(i)}/>
                  }
